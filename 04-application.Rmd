...
```{r access-plot, fig.cap="Relationship between travel time, travel-time based buffer, and logsum access value for block groups in Utah County.", dev = "tikz", dev.args=list(pointsize=10)}
left_join(
  access_bin %>%
...
meaninc_nobuf <- weighted.mean(no_buffer$income, no_buffer$population, na.rm = TRUE)
meaninc_ls <- weighted.mean(no_logsum$income, no_logsum$population, na.rm = TRUE)

nhh_nobuf <- sum(no_buffer$lowincome * .01 * no_buffer$households)
nhh_nols  <- sum(no_logsum$lowincome * .01 * no_logsum$households)
```
...
boundary of all three resource buffers. At the same time, `r nhh_nols` households
...
incomes of these two groups is almost identical however, with \$`r meaninc_nobuf`
for the buffer-based measure and \$`r meaninc_ls` for the utility-based version.
